...
const LabelVisible = ({defaultVisible = false, label = "", tip = "", copy = false}) => {
    ...
    return (
            <span >
                <Tooltip title={tip}>
                    <span onClick={handleClick} style={copy ? {cursor: "pointer"} : {}}>{visible || label && label.length < 9 ? label : label && label.slice(0, 4)+label.slice(4, -4).replace(/\w/g, '*')+label.slice(-4)}</span>
                </Tooltip>
                &nbsp;
