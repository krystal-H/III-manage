...
class Table extends Component<TableProps> {
  ...
  columns = [
    ...
    },
    {
      ...
      dataIndex: "keepUsers",
      key: "keepUsers",
      render: (val: any = nullTag) => this.checkLoading(TabMap.KEEPUSERS, val)
    }
  ];
  ...
  getList = () => {
    ...
    const gn = (d: string) => +d.slice(-2).replace(/^0*/, "") || 0;
    ...
    keepUsersList.forEach((d: dataItemProps) => setMap(d, TabMap.KEEPUSERS));
    list = Array.from(dates)
      .sort((a: string, b: string) => gn(a) - gn(b))
      .map((d: string) => maps[d]);
    return list;
  };
