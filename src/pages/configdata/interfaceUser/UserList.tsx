...
class UserList extends React.Component<IProps, IState> {
    ...
    render() {
        ...
        const columns = [
            ...
            {
                title: "所属客户ID", dataIndex: "parentId", width: "120px", render: (item: number) => {
                    return (<span>{item === -1 ? "系统" : item}</span>);
                }
            },
            { title: "账户角色", dataIndex: "roleName", width: "15%", render: (item: string) => (
                <span title={item}>{item}</span>)
            },
            { title: "账户备注", dataIndex: "remark", width: "18%", render: (item: string) => <span title={item}>{item}</span> },
            {
                title: "账户状态", dataIndex: "status", width: "100px", render: (item: number) => (
